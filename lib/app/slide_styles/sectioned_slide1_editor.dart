...
class __SectionedSlide1State extends State<SectionedSlide1Editor> {
  ...
  Future<void> initializeSlideFontSize() async{
    ...
    for( i = 0; i < controller.slideSectionContentsTextProperties[widget.index].length ; i++){
    if(controller.slideSectionContentsTextProperties[widget.index][i].fontSize == 0){
 controller.slideSectionContentsTextProperties[widget.index][i].fontSize = 0.014;
//  controller.slideSectionContentsTextProperties[widget.index][i].fontColor = Color(widget.slidePallet.bigTitleTColor);
      ...
      if (text.length <= 140) {
    controller.slideSectionContentsTextProperties[widget.index][i].fontSize = 0.018;
      } else if (text.length <= 180) {
    controller.slideSectionContentsTextProperties[widget.index][i].fontSize = 0.016;
      } else if (text.length <= 220) {
    controller.slideSectionContentsTextProperties[widget.index][i].fontSize = 0.0145;
      }
    }
    }
    

    if(controller.slideSectionHeadersTextProperties[widget.index][i].fontSize == 0){
    controller.slideSectionHeadersTextProperties[widget.index][i].fontSize = 0.014;
    ...
    if (widget.mySlide.slideSections[i].sectionHeader != null) {
      ...
      if (text.length <= 140) {
    controller.slideSectionHeadersTextProperties[widget.index][i].fontSize = 0.018;
      } else if (text.length <= 180) {
    controller.slideSectionHeadersTextProperties[widget.index][i].fontSize = 0.016;
      } else if (text.length <= 220) {
    controller.slideSectionHeadersTextProperties[widget.index][i].fontSize = 0.0145;
      }
  ...
  fontSizeWorker = ever(controller.currentFontSize, (_){
    ...
    switchViewStateWorker = ever(controller.switchViewState, (_){
      developer.log("widget font size slide titles : ${widget.slidePallet.slideTitlesTextProperties![widget.index].fontSize!}");
      setState((){});
    });
  ...
  developer.log("This is index in slide 1 ${widget.index}");
   developer.log(" This is the title color in slide 1: ${controller.slideTitlesTextProperties[widget.index].fontColor!}");
   developer.log(" This is the title Size slide 1: ${controller.slideTitlesTextProperties[widget.index].fontSize!}");
   developer.log(" This is the title Weight slide 1: ${controller.slideTitlesTextProperties[widget.index].fontWeight!}");
  ...
  Container _slideSection(int i) {
    ...
    sectionHeaderFontSize.value = widget.size.width * (controller.switchViewState.value  ? widget.slidePallet.slideSectionHeadersTextProperties![widget.index][i].fontSize! : controller.slideSectionHeadersTextProperties[widget.index][i].fontSize!);
    sectionContentFontSize.value = widget.size.width * (controller.switchViewState.value ? widget.slidePallet.slideSectionContentsTextProperties![widget.index][i].fontSize! :  controller.slideSectionContentsTextProperties[widget.index][i].fontSize!);
    ...
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          controller.textForSlideSectionHeaderWithGestureDetector(controller.slideSectionHeaders, widget.index, i, controller.slideSectionHeadersTextProperties, controller.myEditedPresentation, sectionHeaderFontSize, widget.slidePallet),
          verticalSpace(widget.size.height * 0.01),
          controller.textForSlideSectionContentWithGestureDetector(controller.slideSectionContents, widget.index, i, controller.slideSectionContentsTextProperties, controller.myEditedPresentation, sectionContentFontSize, widget.slidePallet),
        ],
      ),
    );
  }

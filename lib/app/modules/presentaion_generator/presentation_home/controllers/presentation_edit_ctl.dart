...
class PresentationEditCtl extends GetxController {
  ...
  late RxList<TextEditingController> slideTitles = <TextEditingController>[].obs;
  late RxList<List<TextEditingController>> slideSectionHeaders = <List<TextEditingController>>[].obs;
  late RxList<List<TextEditingController>> slideSectionContents = <List<TextEditingController>>[].obs;
  ...
  void onInit() {
    ...
    developer.log("this is slide pallet id in edit ctl: ${slidePallet.value.palletId}");
  ...
  void initializeSlidesTextController(){
    ...
    slideTitles.value = myEditedPresentation.value.slides.map(
      (slides){
...
// Example: slideSectionHeaders[1][1] accesses the TextEditingController 
     slideSectionHeaders.value = myEditedPresentation.value.slides.map(
      (slides){
...
// Example: slideSectionContents[1][1] accesses the TextEditingController 
    slideSectionContents.value = myEditedPresentation.value.slides.map(
      (slides){
      return slides.slideSections.map(
        (slideSection){
  ...
  void setFontValue(bool isIncrease){
      double newFontSize = increaseOrDecreaseFontSize(isIncrease, currentFontSize.value);
    if(isTitle.value){
    ...
    // currentPallet.titleFontSize = newFontSize;
      ...
      slideSectionContentsFontValue[firstIndexOfFont.value][secondIndexOfFont.value].value = newFontSize;
      ...
      slideSectionHeadersFontValue[firstIndexOfFont.value][secondIndexOfFont.value].value = newFontSize;
  ...
  void setSlidesText(String value){
    if(isTitle.value){
      ...
      slideSectionContents[firstIndexOfFont.value][secondIndexOfFont.value].text = value;
      currentText.value = value;
    }
    else if(isSectionHeader.value){
      slideSectionHeaders[firstIndexOfFont.value][secondIndexOfFont.value].text = value;
      currentText.value = value;
    }
    else{
    } 
  ...
  void toggleVisibilityTextEditor(bool toggle){
    ...
    developer.log("Toggle bottom text eDITOR visibility ${isBottomNavbarTextEditorVisible.value}");
  }
  void toggleVisibilityBottomNavbarTextField(bool toggle){
    isBottomNavbarTextFieldVisible.value = toggle;
    developer.log("Toggle bottom text field visibility ${isBottomNavbarTextFieldVisible.value}");
  }
  void toggleVisibilityFontSizeProvider(bool toggle){
    isFontSizeProviderVisible.value = toggle;
    developer.log("Toggle bottom Font size visibility ${isFontSizeProviderVisible.value}");
  }

  Future<void> saveSlides(int slideIndex) async{
    List<SlideSection> slides = myEditedPresentation.value.slides[slideIndex].slideSections;
    String slideTitle = myEditedPresentation.value.slides[slideIndex].slideTitle;
    // print("Before editing ${slides.forEach((e){e.sectionContent;})}");
   slides.forEach((e){print("Before ${e.sectionContent}");});
    ...
    slides.forEach((e){print(" After ${e.sectionContent}");});

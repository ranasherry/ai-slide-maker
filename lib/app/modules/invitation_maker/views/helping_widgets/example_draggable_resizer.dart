...
class ExampleResizer extends StatefulWidget {
  const ExampleResizer({Key? key}) : super(key: key);

  @override
  State<ExampleResizer> createState() => ExampleResizerState();
}

class ExampleResizerState extends State<ExampleResizer> {
  double volume = 1.0;

  String ratingImg = AppImages.PPT_BG1;
                  ...
                  onValueChange: ((rating) {
                    setState(() {
                      if (rating >= 0 && rating < 20) {
                        ratingImg = AppImages.PPT_BG1;
                      }
                      if (rating >= 20 && rating < 40) {
                        ratingImg = AppImages.PPT_BG2;
                      }
                      if (rating >= 40 && rating < 60) {
                        ratingImg = AppImages.PPT_BG3;
                      }
  ...
  _musicDialar() => Container(
      ...
      child: Image.asset(AppImages.mainIcon));
  _emoji() => Container(
        height: 120,
        width: 120,
        decoration: BoxDecoration(
  ...
  _speaker() => Container(
        ...
        width: speakerVal + 60,
        decoration: const BoxDecoration(
            image: DecorationImage(image: AssetImage(AppImages.mainIcon)),
            shape: BoxShape.circle),
      );
}
